...
public class VerifyOtpApi extends GenericRequestHandler<VerifyOtpModel> {
    private static ApiInterface authService;
    private Context context;
    private FragmentActivity activity;
    private Map<String, RequestBody> bodyMap;

    public static VerifyOtpApi createInstance(Map<String, RequestBody> bodyMap, Context context, FragmentActivity activity)
    {
        VerifyOtpApi instance = new VerifyOtpApi();
    ...
    public LiveData<DataWrapper<VerifyOtpModel>> onApiRequest() {
        return doRequest(context,activity);
    }

    @Override
    protected Call<VerifyOtpModel> makeRequest() {
        return authService.verifyOtp(bodyMap);
    }
}