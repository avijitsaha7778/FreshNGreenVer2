...
public abstract class GenericRequestHandlerForGetToken<R> {
        ...
        makeRequest().enqueue(new ApiCallback<R>() {
            ...
            protected void handleError(Response<R> response) {
                if (response.errorBody() != null) {
                    dataWrapper.setError(new ApiErrorHandlerForGetTokenApi<R>().handleError(response,context,activity));
                }
                liveData.setValue(dataWrapper);
            }
